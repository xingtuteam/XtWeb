...
function _addressToHex(value) {
    return XtWeb.address.toHex(value).replace(ADDRESS_PREFIX_REGEX, '0x');
}
...
export function encodeParams(types, values) {
    ...
    for (let i = 0; i < types.length; i++) {
        if (types[i] === 'address') {
            values[i] = XtWeb.address.toHex(values[i]).replace(ADDRESS_PREFIX_REGEX, '0x');
        }
    }
...
export function decodeParamsV2ByABI(funABI, data) {
  ...
  const convertAddresses = addrArr => {
    if (Array.isArray(addrArr)) {
      ...
      return XtWeb.address.toHex(addrArr)
    }
  };
  ...
  const decodeResult = (outputs = [], result) => {
    if (outputs.length)
      outputs.forEach((output, i) => {
        ...
        if (result[i])
          if (type === 'address') {
            result[i] = XtWeb.address.toHex(result[i]);
            if(name) result[name] = XtWeb.address.toHex(result[name]);
          }
          else if (type.match(/^([^\x5b]*)(\x5b|$)/)[0] === 'address[') {
